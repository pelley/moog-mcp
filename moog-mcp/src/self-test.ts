...
eq(
  "positionToCC: triangle = 0",
  positionToCC(wave, "triangle"),
  0,
);
eq(
  "positionToCC: narrow-pulse = 127",
  positionToCC(wave, "narrow-pulse"),
  127,
);
eq(
  "positionToCC: middle waveform sits in middle",
  positionToCC(wave, "reverse-sawtooth"),
  Math.round((3 * 127) / 6),
);
...
const ccs = CONTROL_SURFACE.map((c) => c.defaultCC).filter(
