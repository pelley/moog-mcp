...
function controlToolName(spec) {
    return `set_${spec.id}`;
}
function buildControlTool(spec) {
    const desc = [
        spec.description,
        "",
...
const PERFORMANCE_TOOLS = [
    {
        name: "play_note",
        description: "Play a single note for a given duration. Note can be a name like 'C4', 'F#3', 'Bb-1' or a raw MIDI integer 0–127.",
        inputSchema: {
            type: "object",
            properties: {
                note: {
    ...
    {
        name: "play_chord",
        description: "Play multiple notes simultaneously for a given duration. Useful even though the Model D app caps polyphony at 4 voices.",
        inputSchema: {
            type: "object",
            properties: {
                notes: {
    ...
    {
        name: "play_sequence",
        description: "Schedule a timed sequence of MIDI events (notes, CC changes, pitch bends). Returns a sequence id immediately; events fire on a timer. Use this for ambient passages, slow filter sweeps, evolving textures, etc.",
        inputSchema: {
            type: "object",
            properties: {
                events: {
                    type: "array",
                    description: "Event list, each with `at_ms` (offset from start). Kinds: 'note', 'cc', 'pitch_bend'.",
                    items: {
                        type: "object",
                        properties: {
                            ...
                            control: {
                                type: "string",
                                description: "Either a control id (e.g. 'filter_cutoff') OR omit and supply `controller`.",
                            },
    ...
    {
        name: "panic",
        description: "Send All Notes Off + manual note-off for every held note, and cancel all running sequences. Use this if anything sounds stuck.",
    ...
    },
    {
        ...
        description: "List all MIDI output ports visible to the OS (so you can verify the Model D app sees the virtual port).",
        inputSchema: { type: "object", properties: {}, additionalProperties: false },
    },
    {
    ...
    {
        name: "send_raw_cc",
        description: "Escape hatch: send a raw MIDI Control Change message (controller 0–127, value 0–127).",
        inputSchema: {
            type: "object",
            properties: {
                controller: { type: "integer", minimum: 0, maximum: 127 },
...
function playNote(args) {
    const note = parseNoteFlexible(args.note);
    const duration_ms = args.duration_ms ?? 500;
...
function playChord(args) {
    const rawNotes = args.notes;
    const notes = rawNotes.map(parseNoteFlexible);
    const duration_ms = args.duration_ms ?? 800;
    const velocity = args.velocity ?? 100;
    const channel = args.channel;
    for (const n of notes)
        engine.noteOn(n, velocity, channel);
    setTimeout(() => {
        for (const n of notes)
            engine.noteOff(n, channel);
    }, duration_ms);
    return ok(`Played chord [${notes.map(noteName).join(", ")}] for ${duration_ms}ms.`);
}
function playSequence(args) {
    const rawEvents = args.events;
    const events = rawEvents.map((e) => translateSeqEvent(e));
    ...
    return ok(`Scheduled sequence "${id}" with ${events.length} events. Total duration ~${horizon}ms.`);
}
function translateSeqEvent(e) {
    const at_ms = e.at_ms;
    ...
    if (kind === "note") {
        return {
            kind: "note",
            atMs: at_ms,
            note: parseNoteFlexible(e.note),
            durationMs: e.duration_ms ?? 500,
            velocity: e.velocity,
            channel,
        };
    }
    if (kind === "cc") {
        ...
        if (e.control !== undefined) {
            ...
            const spec = findControl(e.control);
            if (spec.kind === "modWheel") {
                controller = 1;
            }
            else if (spec.kind === "pitchWheel") {
                throw new Error("pitchWheel cannot be sent as a CC; use kind: 'pitch_bend' instead.");
            }
 ...
 */
function resolveValueForControl(spec, args) {
    if (typeof args.cc_value === "number") {
        if (!Number.isInteger(args.cc_value) ||
            args.cc_value < 0 ||
            args.cc_value > 127)
    ...
    switch (spec.kind) {
        ...
        case "switchN":
            if (typeof args.position !== "string")
                throw new Error(`Expected position one of: ${spec.positions?.join(", ")}`);
            return positionToCC(spec, args.position);
        case "pitchWheel":
            throw new Error("Pitch wheel uses pitch-bend, not CC. Use the set_pitch_wheel tool path.");
    }
}
function setControl(id, args) {
    ...
    return ok(`${spec.panelLabel} -> CC${spec.defaultCC} = ${cc}` +
...
function cancelSequence(args) {
    const id = args.sequence_id;
    const cancelled = engine.cancelSequence(id);
    return ok(cancelled ? `Cancelled ${id}.` : `No sequence ${id} found.`);
}
function doPanic() {
    const r = engine.panic();
    return ok(`Panic: released ${r.releasedNotes} note(s), cancelled ${r.cancelledSequences} sequence(s).`);
}
function listControls(args) {
...
function getActiveNotes() {
    ...
    if (active.length === 0)
        return ok("No notes currently held.");
    return ok("Active notes:\n" +
        active
            .map((a) => `  ${noteName(a.note)} (MIDI ${a.note}) on ch${a.channel}, held ${a.heldForMs}ms`)
            .join("\n"));
}
function sendRawCC(args) {
